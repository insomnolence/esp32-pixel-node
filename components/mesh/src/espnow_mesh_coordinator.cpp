...
#include "mesh/adaptive/topology_manager.h"
#include "mesh/adaptive/router.h"
    ...
    , tracker_mutex(nullptr)
{
    ...
    if (tracker_mutex == nullptr) {
        ESP_LOGE("BoundedPacketTracker", "Failed to create tracker mutex");
    }
}

BoundedPacketTracker::~BoundedPacketTracker() {
    if (tracker_mutex != nullptr) {
        vSemaphoreDelete(tracker_mutex);
        tracker_mutex = nullptr;
    }
}

bool BoundedPacketTracker::isPacketSeen(uint32_t packet_id) {
    // Try to acquire mutex with short timeout to avoid blocking callbacks
    // If we can't get the lock, assume packet is new (better to process duplicate than drop)
    if (!tracker_mutex || xSemaphoreTake(tracker_mutex, pdMS_TO_TICKS(5)) != pdTRUE) {
        return false; // Can't check, assume new packet
    }
...
ESPNowMeshCoordinator::ESPNowMeshCoordinator() 
    ...
    , discovery_wait_active(false)
    , discovery_wait_start_time(0)
...
void ESPNowMeshCoordinator::handleReceivedPacket(const uint8_t *mac_addr, const uint8_t *data, int len) {
    ...
    if (mesh_packet->data_len > max_data_len) {
        // Consolidate error info into single log to minimize callback delay
        ESP_LOGE(TAG, "Packet size error: data_len=%d > max=%zu, from " MACSTR ", ID=0x%08lX, type=%d",
                 mesh_packet->data_len, max_data_len, MAC2STR(mac_addr),
                 mesh_packet->packet_id, mesh_packet->packet_type);
    ...
    ESP_LOGD(TAG, "Received mesh packet: ID=0x%08lX, type=%d, TTL=%d, size=%d",
        ...
        case MeshPacketType::ROOT_ANNOUNCEMENT:
            ...
            if (mesh_packet->data_len >= 8) {
                ...
                if (announcing_node_has_ble) {
                    ...
                    if (current_role == NodeRole::MESH_ROOT_AUTONOMOUS) {
                        ESP_LOGI(TAG, "Stepping down from autonomous root - BLE root 0x%04X detected", announcing_node_id);
                        transitionToRole(NodeRole::MESH_CLIENT);
                        election_timer = esp_timer_get_time() / 1000 + 60000; // Wait 60 seconds before next election attempt
                    }
                    // BLE-to-BLE priority comparison - newer BLE connection wins
                        ...
                        if (they_have_newer_ble) {
                            ESP_LOGI(TAG, "Stepping down - Node 0x%04X has newer BLE (%lu vs %lu ms)",
                    ...
                    // Autonomous-to-autonomous conflict resolution
                    // If we're both autonomous roots, the one with higher priority (lower node ID) should win
                    if (current_role == NodeRole::MESH_ROOT_AUTONOMOUS) {
                        if (announcing_node_id < node_id) {
                            ESP_LOGI(TAG, "Stepping down - Node 0x%04X has higher priority than 0x%04X",
...
void ESPNowMeshCoordinator::checkForRootElection() {
    ...
    if (current_role != NodeRole::MESH_CLIENT) {
        ...
        // Cancel any active discovery wait if role changed
        discovery_wait_active = false;
        return;
    }
    ...
    if (ble_connected) {
        ...
        // Cancel any active discovery wait if BLE connected
        discovery_wait_active = false;
        return;
    }
    ...
    // Handle non-blocking discovery wait state machine
    if (discovery_wait_active) {
        // Check if we heard from root during discovery wait
        if (heard_from_root) {
            ESP_LOGI(TAG, "Other nodes detected during discovery - not alone");
            discovery_wait_active = false;
            // Other nodes detected - start full advanced election
        ...
        // Check if 2-second discovery wait period has elapsed
        if (current_time - discovery_wait_start_time >= 2000) {
            discovery_wait_active = false;

            // Discovery wait complete - if still no root heard, become autonomous root
            if (!heard_from_root) {
                ESP_LOGI(TAG, "Single node scenario detected - becoming autonomous root");
        ...
        // If discovery wait is still active and not elapsed, just return to let main loop continue
        return;
    }

    // If election timer expired and no root heard, start discovery wait
    if (current_time >= election_timer && !heard_from_root && !ble_connected) {
        ...
        // Start non-blocking discovery wait
        discovery_wait_active = true;
        discovery_wait_start_time = current_time;
        // Return immediately - discovery wait will be checked on next call
        return;
    }
}
...
int8_t ESPNowMeshCoordinator::getAverageNeighborRSSI() const {
    if (neighbor_manager) {
        return neighbor_manager->getAverageRSSI();
    }
    return -100; // No neighbors
}
