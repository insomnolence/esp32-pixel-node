...
void AdaptiveRouter::updatePrimaryRoute(RouteDestination* dest) {
    ...
    while (best_index < dest->route_count && !dest->routes[best_index].is_active) {
        best_index++;
    }

    // No active routes found
    if (best_index >= dest->route_count) {
        dest->primary_route_index = 0; // Keep at 0 but all routes are inactive
        return;
    }

    // Compare remaining routes to find the best active one
    for (uint8_t i = best_index + 1; i < dest->route_count; ++i) {
