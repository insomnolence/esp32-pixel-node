...
void MarchPattern::Update(LEDStrip *strip, led_time_t offset) {
    ...
    if (m_level[0] < 2) {
        ESP_LOGW("MarchPattern", "MarchPattern requires level[0] >= 2, got %u", m_level[0]);
        return;
    }
...
void GradientPattern::Loop(LEDStrip *strip, led_time_t offset) {
    ...
    if (mp1 && mp2) {
        ...
        if (temp_mp1 && temp_mp2) {
            ...
            // Update arrays - thread-safe because all pattern access is serialized through LED task queue
